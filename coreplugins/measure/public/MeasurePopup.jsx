...
export default class MeasurePopup extends React.Component {
  ...
  calculateVolume() {
    ...
    let layers = this.getLayersAtCoords(
      L.latLng(lastCoord.dd.y, lastCoord.dd.x)
    );
        ...
        $.ajax({
          ...
          .done((result) => {
            if (result.celery_task_id) {
              ...
              this.setState({
                error: interpolate(_("Invalid response: %(error)s"), {
                  error: result,
                }),
              });
            }
  ...
  render() {
    ...
    return (
      <div className="plugin-measure popup">
        ...
        {volume === null && !error && (
          <p>
            {_("Volume:")} <i>{_("computing…")}</i>{" "}
            <i className="fa fa-cog fa-spin fa-fw" />
          </p>
        )}
        {typeof volume === "number" && (
          <p>
            {_("Volume:")} {volume.toFixed("2")} {_("Cubic Meters")} (
            {(volume * 35.3147).toFixed(2)} {_("Cubic Feet")})
          </p>
        )}
        {error && (
          <p>
        ...
        <a
          href="#"
          onClick={this.exportMeasurement}
          className="export-measurements"
        >
          <i className="fa fa-download"></i> {_("Export to GeoJSON")}
        </a>
      </div>
    );
  }
