...
export function addTempLayer(file, cb) {
  ...
  if (file && file.size > maxSize) {
    let err = {};
    err.message = interpolate(_("%(file)s is bigger than 5 MB."), {
      file: file.name,
    });
    cb(err);
  } else {
    ...
    if (isZipFile) {
      ...
      reader.onload = function () {
        if (reader.readyState != 2 || reader.error) {
          ...
          shp(reader.result)
            ...
            .catch(function (err) {
              err.message = interpolate(
                _("Not a proper zipped shapefile: %(file)s"),
                { file: file.name }
              );
              cb(err);
            });
      ...
      reader.onload = function () {
        try {
          ...
          err.message = interpolate(_("Not a proper JSON file: %(file)s"), {
            file: file.name,
          });
          cb(err);
        }
