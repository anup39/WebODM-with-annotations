...
class ModelView extends React.Component {
  ...
  pointCloudFilePath = (cb) => {
    ...
    const entwinePointCloud =
      this.assetsPath() + "/entwine_pointcloud/ept.json";
  ...
  hasTexturedModel = () => {
    return (
      this.props.task.available_assets.indexOf("textured_model.zip") !== -1
    );
  };

  getTexturedModelType = () => {
    if (this.props.task.available_assets.indexOf("textured_model.glb") !== -1)
      return "gltf";
    else return "obj";
  };
  ...
  objFilePath = (cb) => {
    ...
    const geoUrl =
      this.texturedModelDirectoryPath() + "odm_textured_model_geo.obj";
    const nongeoUrl =
      this.texturedModelDirectoryPath() + "odm_textured_model.obj";
  ...
  mtlFilename = (cb) => {
    ...
    const geoUrl =
      this.texturedModelDirectoryPath() + "odm_textured_model_geo.mtl";
    ...
    viewer.loadGUI(() => {
      ...
      if (this.hasTexturedModel()) {
        window.ReactDOM.render(
          <TexturedModelMenu toggleTexturedModel={this.toggleTexturedModel} />,
          $("#textured_model_button").get(0)
        );
      } else {
        $("#textured_model").hide();
      ...
      if (this.hasCameras()) {
        window.ReactDOM.render(
          <CamerasMenu toggleCameras={this.toggleCameras} />,
      ...
      if (!this.props.public) {
        ...
        window.ReactDOM.render(
          <SetCameraView viewer={viewer} task={this.props.task} />,
          $scv.get(0)
        );
      }
    });
  ...
  loadCameras() {
    ...
    if (this.hasCameras()) {
      ...
      this.loadGltf("/static/app/models/camera.glb", (err, gltf) => {
        ...
        fileloader.load(
          `/api/projects/${task.project}/tasks/${task.id}/download/shots.geojson`,
          (data) => {
            ...
            geojson.features.forEach((feat) => {
              ...
              cameraMesh.matrix.set(
                ...getMatrix(
                  feat.properties.translation,
                  feat.properties.rotation,
                  scale
                ).elements
              );
  ...
  render() {
    ...
    return (
      <div className="model-view">
        ...
        <div
          className="container potree_container"
        ...
        <div
          className={
            "model-action-buttons " + (this.state.modalOpen ? "modal-open" : "")
          }
        >
