...
export default class LayersControlLayer extends React.Component {
  ...
  componentDidUpdate(prevProps, prevState) {
    ...
    if (
      prevState.formula !== this.state.formula ||
  ...
  handleSelectFormula = (e) => {
    ...
    if (algo && algo["filters"].indexOf(bands) === -1)
      bands = algo["filters"][0]; // Pick first
  ...
  updateHistogram = () => {
    ...
    this.updateHistogramReq = $.getJSON(
      Utils.buildUrlWithQuery(this.meta.metaUrl, this.getLayerParams())
    )
      .done((mres) => {
  ...
  getLayerParams = () => {
    const { colorMap, formula, bands, hillshade } = this.state;
                ...
                <div className="col-sm-9 ">
                  {histogramLoading ? (
                    ...
                    <select
                      ...
                      value={formula}
                      onChange={this.handleSelectFormula}
                    >
                      {algorithms.map((a) => (
                        <option
                          key={a.id}
                ...
                <div className="col-sm-9 ">
                  {histogramLoading ? (
                    ...
                    <select
                      className="form-control"
                      value={bands}
                      onChange={this.handleSelectBands}
                    >
                      {algo.filters.map((f) => (
              ...
              <div className="row form-group form-inline">
                ...
                <div className="col-sm-9 ">
                  <select
                    className="form-control"
                    value={hillshade}
                    onChange={this.handleSelectHillshade}
                  >
