...
class Metadata(TaskNestedView):
    def get(self, request, pk=None, project_pk=None, tile_type=""):
            ...
            with COGReader(raster_path) as src:
                ...
                if expr is not None:
                    if boundaries_cutline is not None:
                        data, mask = src.preview(expression=expr, vrt_options={
                                                 'cutline': boundaries_cutline})
                    else:
                    ...
                    metadata = RioMetadata(
                        statistics=stats, **src.info().dict())
                else:
                    if (boundaries_cutline is not None) and (boundaries_bbox is not None):
                        metadata = src.metadata(pmin=pmin, pmax=pmax, hist_options=histogram_options,
                                                nodata=nodata, bounds=boundaries_bbox, vrt_options={'cutline': boundaries_cutline})
                    else:
                        metadata = src.metadata(
                            pmin=pmin, pmax=pmax, hist_options=histogram_options, nodata=nodata)
        ...
        info['name'] = task.name
        info['scheme'] = 'xyz'
        info['tiles'] = [get_tile_url(
            task, tile_type, self.request.query_params)]
    ...
    def get(self, request, pk=None, project_pk=None, tile_type="", z="", x="", y="", scale=1, ext=None):
        ...
        with COGReader(url) as src:
            ...
            if hillshade is not None:
                ...
                elevation[tilesize*2:tilesize*3,
                          tilesize*2:tilesize*3] = nodata

                intensity = ls.hillshade(
                    elevation, dx=dx, dy=dy, vert_exag=hillshade)
                intensity = intensity[tilesize:tilesize *
                                      2, tilesize:tilesize * 2]

            if intensity is not None:
                rgb = tile.post_process(in_range=(rescale_arr,))
                rgb_data = rgb.data[:, tilesize:tilesize *
                                    2, tilesize:tilesize * 2]
                if colormap:
                    rgb, _discard_ = apply_cmap(
                        rgb_data, colormap.get(color_map))
                ...
                if rgb is not None:
                    mask = tile.mask[tilesize:tilesize *
                                     2, tilesize:tilesize * 2]
                    return HttpResponse(
